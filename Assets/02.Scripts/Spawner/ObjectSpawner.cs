...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        SpawnObjects(enemyPrefabs, maxEnemy, activeEnemies);
        SpawnObjects(resourcePrefabs, maxResource, activeResources);

        StartCoroutine(CheckAndRespawn()); // 지속적으로 리스폰 체크
    }

    private void SpawnObjects(GameObject[] prefabs, int count, List<GameObject> activeList)
    {
    ...
    private IEnumerator CheckAndRespawn()
    {
        while (true)
        {
            ...
            RespawnIfNeeded(enemyPrefabs, maxEnemy, activeEnemies);
            RespawnIfNeeded(resourcePrefabs, maxResource, activeResources);
        }
    }

    private void RespawnIfNeeded(GameObject[] prefabs, int maxCount, List<GameObject> activeList)
    {
        // 리스트에서 삭제된 오브젝트 정리
        ...
        if (missingCount > 0)
        {
            ...
            SpawnObjects(prefabs, missingCount, activeList);
        }
    }
